...
export default function Payments() {
  ...
  const calculateStats = () => {
    ...
    const avgMonthly = transactions.length > 0 ? totalEarnings / transactions.length : 0;
    ...
    return (
      <div className="w-full min-h-screen px-2 pt-6">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">{t('seller:recentOrders.loading', 'Loading...')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="w-full min-h-screen px-2 pt-6">
        <div className="text-center text-red-600">
          <p>{t('seller:recentOrders.empty', 'Error loading payment history')}</p>
          <button
            onClick={() => refetch()}
    ...
    <div className="w-full min-h-screen px-2 pt-6">
      ...
      <div>
        ...
        <p className="text-[#A78BFA] text-sm mt-1">
          {t('seller:payments.subtitle', 'Track your earning and transaction history')}
        </p>
      </div>
      ...
      >
        ...
        <p className="text-sm text-[#8A72BE] mt-1">
          ...
          <span className="text-[#8A72BE]">{formatAmount(stats.availableBalance)}</span>
        </p>
        <div className="flex gap-4 mt-3">
          ...
          <span className="text-xs px-2 py-1 bg-[#FFF1E2] text-[#F39C4A] rounded-full">
            {t('seller:payments.summary.pending', 'Pending')}: {stats.pendingCount}
          </span>
          <span className="text-xs px-2 py-1 bg-[#FFE2E2] text-[#E53E3E] rounded-full">
            {t('seller:payments.summary.canceled', 'Canceled')}: {stats.canceledCount}
          </span>
          <span className="text-xs px-2 py-1 bg-[#EBF4FF] text-[#3182CE] rounded-full">
            {t('seller:payments.summary.total', 'Total')}: {stats.totalTransactions}
          </span>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <div>
              <span className="text-gray-600">
                {t('seller:payments.pagination.totalTransactions', 'Total Transactions')}:
              </span>
            ...
            <thead>
              <tr className="text-left bg-[#F9F5FF] text-[#A78BFA] text-sm">
                <th className="py-3 px-4 font-medium">
                  {t('seller:wallet.transactions.table.transaction', 'TRANSACTION ID')}
                </th>
                <th className="py-3 px-4 font-medium">{t('seller:table.orderId', 'ORDER ID')}</th>
                <th className="py-3 px-4 font-medium">{t('seller:table.amount', 'AMOUNT')}</th>
                <th className="py-3 px-4 font-medium">
                ...
                <th className="py-3 px-4 font-medium">{t('seller:table.status', 'STATUS')}</th>
                <th className="py-3 px-4 font-medium">{t('seller:table.date', 'DATE')}</th>
              </tr>
            </thead>
              ...
              {displayedTransactions.length === 0 ? (
                <tr>
                  <td colSpan="7" className="py-8 text-center text-gray-500">
                    {search
                      ? t('seller:productsPage.pagination.noResults', 'No results found')
                ...
                displayedTransactions.map((txn, idx) => (
                  <tr key={txn._id} className="border-b border-[#EDECF5] hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4 text-[#1D1D1F] font-medium">{txn.orderId}</td>
                    ...
                    <td className="py-4 px-4 text-[#F39C4A]">
                      <div className="text-sm">
                        <div>
                          {t('seller:payments.commission.platformFee', 'Platform fee')}:{' '}
                    ...
                    <td className="py-4 px-4 text-[#1D1D1F]">{formatDate(txn.createdAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
