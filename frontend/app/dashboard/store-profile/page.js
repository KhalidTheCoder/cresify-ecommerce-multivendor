...
export default function StoreProfile() {
  ...
  const { data: productResponse, isLoading: productsLoading } = useMyProductQuery({
    id,
    limit: 10,
    skip: 0,
  });
  ...
  const uploadToImgBB = async (imageFile) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        throw new Error(data.error?.message || t('seller:imageUploadFailed'));
      }
    } catch (error) {
  ...
  const generateStars = (rating) => {
    ...
    for (let i = 0; i < 5; i++) {
      if (i < fullStars) {
        stars.push(<BsStarFill key={i} className="text-[#FFA534] text-[17px]" />);
      } else {
        stars.push(<BsStarFill key={i} className="text-[#E0E0E0] text-[17px]" />);
      }
    }
    return stars;
  };
                  ...
                  <div className="flex flex-wrap gap-6 mb-6">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">
                        {t('seller:storeSince')} {new Date().getFullYear()}
                      </span>
                    </div>
                ...
                <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-xl">
                    ...
                    <div className="text-xs text-gray-500 mt-1">
                      {t('seller:basedOnProducts')} {previewProducts.length} {t('seller:products')}
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-purple-700 font-medium">
                    {t('seller:showingSampleProducts')} {previewProducts.length}{' '}
                    {t('seller:sampleProducts')}
                  </p>
                </div>
                <div className="text-sm text-gray-600">{t('seller:previewMode')}</div>
              </div>
            </div>
                ...
                <div key={product._id} className="group">
                  <article className="bg-white rounded-3xl p-5 shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 flex flex-col h-full">
                    ...
                    <div className="relative mb-5 overflow-hidden rounded-2xl">
                      ...
                      {/* Tag */}
                      <span
                        className={`absolute top-4 left-4 ${product.stock > 10 ? 'bg-[#4CAF50]' : 'bg-[#FF7C7C]'} text-white text-xs rounded-full px-3 py-1.5 font-medium shadow-lg`}
                      >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center justify-between mb-5">
                        ...
                        <div className="text-right">
                          <span
                            className={`inline-block px-3 py-1 text-xs font-medium rounded-full ${product.stock > 10 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-4">
                  {t('seller:storeInformation')}
                </h3>
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <AiOutlineShop className="text-purple-600" />
                    <span className="text-gray-600">
                      {formData.shopName || t('seller:yourStore')}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <AiOutlineEnvironment className="text-purple-600" />
                    <span className="text-gray-600">
                      {formData.address || t('seller:addressPlaceholder')}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <AiOutlinePhone className="text-purple-600" />
                    <span className="text-gray-600">
                      {formData.phoneNumber || t('seller:phonePlaceholder')}
                    </span>
                  </div>
                  <div className="flex items-center gap-3">
                    <AiOutlineMail className="text-purple-600" />
                    <span className="text-gray-600">
                      {formData.email || t('seller:emailPlaceholder')}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-4">
                  {t('seller:storePerformance')}
                </h3>
                <ul className="space-y-2">
                  <li className="text-gray-600">
                    <span className="font-medium">{t('seller:averageRating')}:</span>{' '}
                    {averageRating.toFixed(1)}/5
                  </li>
                  <li className="text-gray-600">
                    <span className="font-medium">{t('seller:sampleProducts')}:</span>{' '}
                    {previewProducts.length}
                  </li>
                  <li className="text-gray-600">
                    <span className="font-medium">{t('seller:storeStatus')}:</span>{' '}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
          <div className="flex-1">
            ...
            <p className="text-purple-600 mt-2 text-lg">{t('seller:storeProfile.subtitle')}</p>
          </div>
            ...
            <div className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-purple-500/10 to-blue-500/10 text-purple-700 rounded-xl">
              ...
              <span className="font-medium">
                {profile?.data?.role === 'seller'
                  ? t('seller:sellerAccount')
                  : t('seller:serviceProvider')}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 md:p-8">
              <div className="flex items-center mb-8">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">{t('seller:storeProfile.identity.subtitle')}</p>
                </div>
              </div>
                      ...
                      <label className="cursor-pointer">
                        <div className="aspect-square rounded-xl border-2 border-dashed border-gray-300 bg-gradient-to-br from-gray-50 to-gray-100 hover:from-gray-100 hover:to-gray-200 transition-all flex flex-col items-center justify-center hover:border-teal-500 group">
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            ({10 - allServicesImages.length} {t('seller:remaining')})
                          </p>
                        </div>
                        <input
                          type="file"
                ...
                {allServicesImages.length > 0 && allServicesImages.length < 10 && (
                  <div className="text-center pt-4">
                    <label className="cursor-pointer inline-flex items-center gap-2 px-6 py-3 bg-white border-2 border-teal-200 text-teal-700 font-medium rounded-xl hover:bg-teal-50 transition-all">
                      <FiUploadCloud />
                      {t('seller:addMoreImages')} ({10 - allServicesImages.length}{' '}
                      {t('seller:remaining')})
                      <input
                        type="file"
                        multiple
          ...
          {activeTab === 'policies' && (
            <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 md:p-8">
              <div className="flex items-center mb-8">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">{t('seller:storePolicies')}</h2>
                  <p className="text-gray-600 mt-1">{t('seller:storePoliciesSubtitle')}</p>
                </div>
              </div>
