/**
 * @file page.js
 * @description Vendor reviews page listing all customer reviews for the seller's products.
  ...
  if (isLoading) {
    return (
      <div className="w-full min-h-screen px-2 pt-6">
        <h1 className="text-[28px] font-semibold text-gray-900">{t('reviews.title')}</h1>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="space-y-2 text-end">
                      ...
                      <p className="text-xs text-[#9838E1]">{formatDate(item.createdAt)}</p>
                    </div>
                  </div>

                  {/* REVIEW TEXT */}
                  <p className="mt-3 text-gray-700 text-[15px] leading-relaxed">{item.review}</p>

                  {/* COMMENT/REPLY SECTION - Only show if reply exists */}
                  {item.reply && (
                    <div className="mt-4 bg-[#f5eaff] text-[#7b43ff] rounded-lg p-4 text-sm">
                      <p className="font-medium text-gray-600 mb-1">{t('reviews.yourReply')}:</p>
                      {item.reply}
                    </div>
                  )}
