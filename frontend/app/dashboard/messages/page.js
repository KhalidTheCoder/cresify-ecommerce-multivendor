/**
 * @file page.js
 * @description Vendor real-time messaging page with conversation list and chat interface.
  ...
  useEffect(() => {
    ...
    const loadOrCreateConversation = async () => {
      try {
        ...
        const convResponse = await fetch(`${base_url}/chat/conversations/${userId}`, {
          credentials: 'include',
        });
        ...
        if (existingConversation) {
          ...
          const openConvResponse = await fetch(`${base_url}/chat/openConversation`, {
            method: 'POST',
            headers: {
  ...
  return (
    <div className="w-full h-screen flex bg-[#F6F6F9]">
      ...
      <div className="w-[300px] border-r border-gray-200 bg-[#F5EEF8] flex flex-col mt-8">
        <div className="p-5">
          <h1 className="text-[28px] font-semibold text-[#8D55DA]">{t('messages.title')}</h1>
          <input
            type="text"
        ...
        <div className="overflow-y-auto flex-1">
          {isLoadingChats ? (
            <div className="px-5 py-3 text-center text-gray-500">{t('chat.loadingChats')}</div>
          ) : chatList?.length > 0 ? (
            chatList.map((chat) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-5 py-3 cursor-pointer relative ${
                    isSelected ? 'bg-[#f0e6fb]' : 'bg-[#F5EEF8] hover:bg-[#f0e6fb]'
                  }`}
                >
                  {isSelected && (
            ...
            <div className="px-5 py-3 text-center text-gray-500 text-sm">{t('chat.noChats')}</div>
          )}
        </div>
      </div>
        ...
        {selectedChat && conversation ? (
            ...
            <div className="flex items-center justify-between px-8 py-5 bg-white border-b">
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <div
                    className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${
                      onlineUsers[otherParticipant?._id] ? 'bg-green-500' : 'bg-gray-400'
                    }`}
                  ></div>
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {onlineUsers[otherParticipant?._id] ? t('chat.online') : t('chat.offline')}
                  </p>
                </div>
              </div>
            </div>
              ...
              {isLoadingMessages ? (
                ...
                <div className="h-full flex items-center justify-center">
                  <div className="text-center text-gray-500">
                    ...
                    <p className="text-sm">{t('chat.startConversationMessage')}</p>
                  </div>
                </div>
              ) : (
                messages.map((msg, index) => {
                  const showAvatar =
                    index === 0 || isMessageFromMe(messages[index - 1]) !== isMessageFromMe(msg);
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isMe && showAvatar && (
                        <Image
                          src={
                            otherParticipant?.image || otherParticipant?.businessLogo || '/user.png'
                          }
                          width={32}
                          height={32}
                      ...
                      >
                        ...
                        >
                          {msg._id?.startsWith('temp-') ? (
                            <span className="italic text-yellow-600">{t('chat.sending')}</span>
                          ) : (
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500">{t('messages.emptyState.subtitle')}</p>
            </div>
          </div>
        )}
      </div>
