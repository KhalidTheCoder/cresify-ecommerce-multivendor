...
const SellerRefundpage = () => {
  ...
  const getProductNames = (items) => {
    if (!items || items.length === 0) return t('seller:refunds.fullOrder', 'Full Order');

    return items
      .map((item) => item.product?.name || t('seller:refunds.product', 'Product'))
      .join(', ');
  };
          ...
          <table className="min-w-full text-left text-[12px] md:text-[13px]">
            <thead>
              <tr className="text-[#9838E1] bg-[#F8F4FD] border-b border-[#F0EEF7] text-xs uppercase tracking-[0.12em]">
                ...
                <th className="py-6 pr-4 font-medium pl-2">{t('seller:table.amount', 'Amount')}</th>
                <th className="py-6 pr-4 font-medium pl-2">{t('seller:refunds.items', 'Items')}</th>
                <th className="py-6 pr-4 font-medium pl-2">{t('seller:table.status', 'Status')}</th>
                <th className="py-6 pr-4 font-medium pl-2">
    ...
    <div className="bg-white rounded-xl p-4 md:p-6">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        <div className="bg-[#FFF1E2] text-[#F39C4A] px-4 py-2 rounded-lg">
          <span className="font-semibold">{t('seller:refunds.summary.pending', 'Pending')}:</span>{' '}
          {refunds.filter((r) => r.status === 'requested' || r.status === 'pending').length}
        </div>
        <div className="bg-[#E2FFE9] text-[#38A169] px-4 py-2 rounded-lg">
          <span className="font-semibold">{t('seller:refunds.summary.approved', 'Approved')}:</span>{' '}
          {refunds.filter((r) => r.status === 'approved' || r.status === 'completed').length}
        </div>
        <div className="bg-[#FFE2E2] text-[#E53E3E] px-4 py-2 rounded-lg">
          <span className="font-semibold">{t('seller:refunds.summary.rejected', 'Rejected')}:</span>{' '}
          {refunds.filter((r) => r.status === 'rejected' || r.status === 'denied').length}
        </div>
      </div>
          ...
          <tbody className="text-[#4B4B63]">
            {refunds.map((refund, idx) => {
              ...
              return (
                <tr
                  key={refund._id}
                  className={`border-b border-[#F5F4FB] ${idx % 2 === 1 ? '' : 'bg-white'}`}
                >
                  ...
                  <td className="pl-2 py-6 pr-4">
                    <span className="bg-[#F8F4FD] text-[#9838E1] px-3 py-1 rounded-full text-xs font-medium">
                      {refund.items?.length || 0} {t('seller:refunds.items', 'items')}
                    </span>
                  </td>
                  <td className="pl-2 py-6 pr-4">
                    <span
                  ...
                  <td className="pl-2 py-6 pr-4 text-[#8C8CA1]">{formatDate(refund.createdAt)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      {pagination.pages > 1 && (
        <div className="flex flex-col md:flex-row md:items-center justify-between mt-6 px-2 gap-4">
          <div className="text-sm text-gray-600">
            {t('seller:productsPage.pagination.showing', 'Showing')} {(page - 1) * limit + 1}{' '}
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-lg text-sm font-medium ${
                    page === pageNum ? 'bg-[#9838E1] text-white' : 'text-gray-600 hover:bg-gray-100'
