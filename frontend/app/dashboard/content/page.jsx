...
import { ArrowRightIcon, ChevronDown, PencilLine, Upload, X } from 'lucide-react';
...
const ArticlesPage = () => {
  ...
  const { data: blogsData, isLoading, error } = useGetBlogsQuery({ page, limit: 9 });
  ...
  return (
    <div className="p-6 bg-gray-50 relative">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-semibold text-gray-800">{t('admin:articles.title')}</h1>
        <button
      ...
      {isLoading && page === 1 && (
        <div className="flex justify-center items-center py-20">
          <div className="text-lg text-gray-600">{t('admin:articles.loading')}</div>
        </div>
      )}

      {error && (
        <div className="flex justify-center items-center py-20">
          <div className="text-lg text-red-600">{t('admin:articles.error')}</div>
        </div>
      )}
      ...
      {!isLoading && !error && blogsData?.data?.length === 0 && (
        <div className="flex flex-col justify-center items-center py-20">
          ...
          <p className="text-gray-600 mb-6">{t('admin:articles.emptyState.description')}</p>
        </div>
      )}
        ...
        <>
                ...
                <div className="bg-white rounded-lg shadow-lg overflow-hidden p-5">
                  <div className="w-full h-48 overflow-hidden rounded-lg pb-4 relative group">
                    <Image
                      src={
                        article.img ||
                        `https://placehold.co/362x240/8736C5/white?text=Article+${index + 1}`
                      }
                      alt={article.title}
                      width={362}
                      height={240}
                  ...
                  <p className="text-sm text-[#AC65EE] font-semibold mb-3">{article.category}</p>
                  <div>
                    <h3
                    ...
                    <div className="flex justify-between items-center mt-4">
                      ...
                      >
                        {t('admin:articles.card.readMore')} <ArrowRightIcon className="w-6 h-6" />
                      </a>
                    </div>
                  </div>
                </div>
          ...
          {blogsData?.pagination?.hasMore && (
            <div className="flex justify-center mt-8">
              ...
              >
                {isLoading ? t('admin:articles.loading') : t('admin:articles.loadMore')}
              </button>
            </div>
          )}
        </>
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 px-6">
              <div>
                ...
                <input
                  ...
                  placeholder={t('admin:articles.modal.fields.titlePlaceholder')}
                ...
                <div className="relative" ref={dropdownRef}>
                  ...
                  >
                    <span className={category ? 'text-gray-900' : 'text-gray-400'}>
                      {category || t('admin:articles.modal.fields.selectCategory')}
                    </span>
                    <ChevronDown
            ...
            <div className="px-6 pb-4">
              ...
              <textarea
                ...
                placeholder={t('admin:articles.modal.fields.detailsPlaceholder')}
                rows="4"
