...
function SummaryCard({ title, value, icon, color, subtitle, onClick }) {
  const colors = {
    green: 'bg-gradient-to-br from-green-50 to-emerald-50 text-green-700 border-green-200',
    blue: 'bg-gradient-to-br from-blue-50 to-indigo-50 text-blue-700 border-blue-200',
    amber: 'bg-gradient-to-br from-amber-50 to-orange-50 text-amber-700 border-amber-200',
    purple: 'bg-gradient-to-br from-purple-50 to-violet-50 text-purple-700 border-purple-200',
  };
    ...
    return (
      <div className="bg-gradient-to-br from-gray-50 to-gray-100 border-2 border-dashed border-gray-300 rounded-xl p-6 text-center">
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-gray-500 mt-3">Secure • Verified • Instant Transfers</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-6">
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-lg text-gray-900">{t('wallet.stripeAccount.title')}</h3>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-xs text-gray-500">
                ...
                {account?.accountId ? `${account.accountId.substring(0, 8)}...` : '-'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white p-3 rounded-lg border">
          <p className="text-xs text-gray-500 mb-1">{t('wallet.stripeAccount.accountType')}</p>
          <p className="font-semibold capitalize">Express</p>
        </div>
        <div className="bg-white p-3 rounded-lg border">
          <p className="text-xs text-gray-500 mb-1">{t('wallet.stripeAccount.currency')}</p>
          <p className="font-semibold uppercase">{account?.currency ?? 'USD'}</p>
        </div>
        <div className="bg-white p-3 rounded-lg border">
          <p className="text-xs text-gray-500 mb-1">{t('wallet.stripeAccount.payoutsEnabled')}</p>
          <p
        ...
        <div className="bg-white p-3 rounded-lg border">
          <p className="text-xs text-gray-500 mb-1">{t('wallet.stripeAccount.requirements')}</p>
          <p
            className={`font-semibold ${
              (account?.requirementsDue?.length || 0) === 0 ? 'text-green-600' : 'text-amber-600'
            }`}
          >
            {(account?.requirementsDue?.length || 0) === 0
              ? 'Complete'
              : `${account.requirementsDue.length} ${t('wallet.stripeAccount.pending')}`}
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg border mb-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-600">{t('wallet.expressDetails.email')}</span>
            <span className="font-medium">{account?.email ?? '-'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">{t('wallet.expressDetails.country')}</span>
            <span className="font-medium">{account?.country ?? '-'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">{t('wallet.expressDetails.defaultCurrency')}</span>
            <span className="font-medium uppercase">
              {account?.defaultCurrency ?? account?.currency ?? 'USD'}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">{t('wallet.expressDetails.chargesEnabled')}</span>
            <span
...
====================================================== */
function StripeAccountForm({ isOpen, onClose, account, onSave, mode = 'connect' }) {
  const { t } = useTranslation('seller');
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 p-3 rounded-lg border border-blue-200 mb-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-blue-800">Stripe Express Account</p>
              </div>
              <p className="text-xs text-blue-700 mt-1">
                You're connecting a Stripe Express account for receiving payments
              </p>
            </div>
      ...
      case 2:
        return (
          <div className="space-y-4">
            <h4 className="font-semibold text-gray-900 mb-4">Business Address</h4>
        ...
        return (
          <div className="space-y-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              This information is securely transmitted to Stripe for Express account payout
              processing.
            </p>
            ...
            <div>
              ...
              <input
                ...
                className={`block w-full rounded-lg border ${
                  errors['bankAccount.accountNumber'] ? 'border-red-300' : 'border-gray-300'
                } px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                placeholder="000123456789"
              />
              {errors['bankAccount.accountNumber'] && (
                <p className="mt-1 text-sm text-red-600">{errors['bankAccount.accountNumber']}</p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={`block w-full rounded-lg border ${
                  errors['bankAccount.routingNumber'] ? 'border-red-300' : 'border-gray-300'
                } px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500`}
                placeholder="021000021"
              />
              {errors['bankAccount.routingNumber'] && (
                <p className="mt-1 text-sm text-red-600">{errors['bankAccount.routingNumber']}</p>
              )}
            </div>
          ...
          {[1, 2, 3].map((stepNumber) => (
            <div key={stepNumber} className="flex flex-col items-center z-10">
              ...
              <span className="text-xs mt-2 text-gray-600">
                {stepNumber === 1 ? 'Basic Info' : stepNumber === 2 ? 'Address' : 'Bank Details'}
              </span>
            </div>
          ))}
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-blue-700 mt-1">
                All information is encrypted and securely transmitted to Stripe Express. We never
                store your bank account details on our servers.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <div className="flex-1 flex gap-3">
            ...
            {step < 3 ? (
              ...
              >
                {loading ? 'Saving...' : mode === 'connect' ? 'Complete Setup' : 'Update Account'}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-md w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-bold text-gray-900">Request Stripe Withdrawal</h3>
            <p className="text-sm text-gray-600">Withdraw funds via Stripe Express</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          {stripeAccount && (
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                <SiStripe className="text-purple-600" />
                <span className="font-medium text-purple-800">Stripe Express Account</span>
              </div>
            ...
            <div className="flex gap-2 mt-2">
              {[100, 250, 500, wallet.balance].map((suggestedAmount) => (
                <button
                  key={suggestedAmount}
                  type="button"
                  onClick={() => setAmount(Math.min(suggestedAmount, wallet.balance).toString())}
          ...
          <div className="flex items-start gap-2 text-xs text-gray-500">
            ...
            <p>
              Withdrawal requests are processed within 2-3 business days via Stripe Express. Minimum
              withdrawal: {wallet.currency}10.
            </p>
          </div>
...
export default function WalletDetailsPage() {
  ...
  const normalizeStripe = (acct) => {
    ...
    return {
      ...
      currency: acct.default_currency || acct.currency || acct.defaultCurrency || null,
      defaultCurrency: acct.default_currency || acct.defaultCurrency || acct.currency || null,
      ...
      requirementsDue:
        acct.requirements?.currently_due || acct.requirementsDue || acct.requirements_due || [],
  ...
  const { data: walletData, isLoading: walletLoading } = useGetWalletQuery(user?.id, {
    skip: !user?.id,
  });
  ...
  const handleStripeAccountSave = async (data) => {
    ...
    try {
      ...
      setStripeAccount(normalizeStripe(updatedWallet.stripeDetails) || normalizeStripe(data));
      ...
      alert('Failed to save Stripe account: ' + (err?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStripeAccountRefresh = async () => {
    ...
    try {
      ...
      alert('Failed to refresh Stripe account: ' + (err?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleWithdrawalSubmit = async (withdrawalData) => {
    ...
    try {
      ...
      const newTransaction = {
        ...
        method: body.method,
        reference: res.payout?.payoutId || `WTH-${Date.now().toString().slice(-6)}`,
      };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <SummaryCard
            ...
            icon={<FaCalendarAlt />}
            color="purple"
            subtitle={`${wallet.payoutFrequency} ${t('wallet.balance.monthlySchedule')}`}
          />
        </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl border p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('wallet.expressInfo.payoutsEnabled')}</span>
                  <span
                    className={`font-semibold ${
                      stripeAccount?.payoutsEnabled ? 'text-green-600' : 'text-red-600'
                    }`}
                  >
                ...
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('wallet.expressInfo.accountType')}</span>
                  <span className="font-semibold">Express</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('wallet.expressInfo.withdrawalFee')}</span>
                  <span className="font-semibold">1.5%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('wallet.expressInfo.processingTime')}</span>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-5">
              ...
              <ul className="space-y-2 text-sm">
                <li className="flex items-center gap-2">
                  ...
                  <span className="text-gray-700">{t('wallet.benefits.bankTransfers')}</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaCheckCircle className="text-green-500" />
                  <span className="text-gray-700">{t('wallet.benefits.lowFee')}</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaCheckCircle className="text-green-500" />
                  <span className="text-gray-700">{t('wallet.benefits.secure')}</span>
                </li>
                <li className="flex items-center gap-2">
                  <FaCheckCircle className="text-green-500" />
                  <span className="text-gray-700">{t('wallet.benefits.support')}</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="bg-white rounded-xl shadow overflow-hidden">
          <div className="px-6 py-4 border-b flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">{t('wallet.transactions.subtitle')}</p>
            </div>
          ...
          {transactions.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-gray-600 mt-1">Your transaction history will appear here</p>
            </div>
          ) : (
                  ...
                  {transactions.map((txn) => (
                    <tr key={txn.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4">
                        <div>
                          <div className="text-sm font-medium text-gray-900">{txn.title}</div>
                          <div className="text-xs text-gray-500">Ref: {txn.transactionId}</div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          {txn.method === 'stripe' ? (
                            <>
                              <SiStripe className="text-purple-500" />
                              <span className="text-sm text-gray-700">Stripe Express</span>
                            </>
                          ) : (
                            <>
                              <FaCreditCard className="text-gray-500" />
                              <span className="text-sm text-gray-700 capitalize">{txn.method}</span>
                            </>
                          )}
                        </div>
                      </td>
