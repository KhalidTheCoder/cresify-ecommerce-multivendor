/**
 * @file page.js
 * @description Admin product moderation page — approve, reject, and remove product listings.
 * @module app/dashboard/admin-dashboard/products/page
 */
...
export default function ProductManagementPage() {
  ...
  const { data: productDetails, isLoading: isProductLoading } = useGetProductByIdQuery(
    selectedProductId,
    {
  ...
  return (
    <div className="min-h-screen w-full px-2 pt-6">
      {/* Header */}
      <div>
        <h1 className="text-[26px] font-semibold text-gray-900">{t('productManagement.title')}</h1>
        <p className="text-sm text-[#9C6BFF] mt-1">{t('productManagement.subtitle')}</p>
      </div>
          ...
          return (
            ...
            >
              <div>
                ...
                <p className="mt-2 text-[22px] font-semibold text-[#F88D25]">{card.value}</p>
              </div>
                ...
                {products.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-3">
                      ...
                      >
                        <option value="">{t('productManagement.table.selectOption')}</option>
                        <option value="rejected">{t('productManagement.status.rejected')}</option>
        ...
        <Dialog.Panel className="bg-white p-6 rounded-xl shadow-lg max-w-md w-full relative">
          ...
          {isProductLoading ? (
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                <span className="font-medium">{t('productManagement.modal.fields.name')} </span>
                {productDetails.data.name}
              </div>

              <div>
                <span className="font-medium">{t('productManagement.modal.fields.seller')} </span>
                {productDetails.data.seller?.name || t('productManagement.table.na')}
              </div>

              <div>
                <span className="font-medium">{t('productManagement.modal.fields.category')} </span>
                {productDetails.data.category}
              </div>

              <div>
                <span className="font-medium">{t('productManagement.modal.fields.price')} </span>$
                {productDetails.data.price}
              </div>

              <div>
                <span className="font-medium">{t('productManagement.modal.fields.stock')} </span>
                {productDetails.data.stock}
              </div>

              <div>
                <span className="font-medium">{t('productManagement.modal.fields.status')} </span>
                <span
                  className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium ${
                    statusColors[productDetails.data.status] || 'bg-gray-100 text-gray-800'
                  }`}
                >
              ...
              <div>
                <span className="font-medium">{t('productManagement.modal.fields.created')} </span>
