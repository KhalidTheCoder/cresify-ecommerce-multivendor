...
export default function ServiceProviderDashboard() {
  ...
  const { data: bookingStats, isLoading: statsLoading } = useGetProviderBookingStatsQuery({
    providerId,
    filter: bookingFilter,
  });

  const { data: todayData, isLoading: todayLoading } = useGetProviderTodaysBookingsQuery(
    session?.user?.id,
  );
  const { data: upcomingData, isLoading: upcomingLoading } = useGetProviderUpcomingBookingsQuery(
    session?.user?.id,
  );
        ...
        <div className="bg-white border border-[#EFE9FF] rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{t('dashboard.stats.avgBookingValue')}</p>
            </div>
          </div>
          <div className="text-xs text-gray-500 mt-2">{t('dashboard.stats.perService')}</div>
        </div>
        ...
        <div className="bg-white border border-[#EFE9FF] rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{t('dashboard.stats.monthlyGrowth')}</p>
            </div>
          </div>
        ...
        <div className="bg-white border border-[#EFE9FF] rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{t('dashboard.stats.services')}</p>
            </div>
          </div>
          <div
          ...
          >
            {overviewData?.providerInfo?.serviceName || t('dashboard.noServiceName')}
          </div>
        </div>
        ...
        <div className="bg-white border border-[#EFE9FF] rounded-2xl p-6 shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{t('dashboard.stats.averageRating')}</p>
            </div>
          </div>
          <div className="text-xs text-gray-500 mt-2">{t('dashboard.stats.basedOnReviews')}</div>
        </div>
      </div>
          ...
          <div className="w-full h-[260px]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={incomeData}>
                ...
                />
                <Tooltip
                  formatter={(value) => [`$${value}`, t('dashboard.charts.earnings')]}
                ...
                <div key={index} className="bg-gray-50 rounded-xl p-4">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs font-medium rounded-full">
                      {service.count} {t('dashboard.servicePerformance.bookings')}
                    </span>
                  </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mt-1">{t('dashboard.bookingsWillAppear')}</p>
          </div>
        ) : (
          items.map((item, i) => (
            <div
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-900 truncate">
                          {item.customerName || item.customer?.fullName || t('dashboard.customer')}
                        </h3>
                        <p className="text-sm text-gray-600 mt-1">
                          {item.serviceName || item.provider?.serviceName || t('dashboard.service')}
                        </p>
                        <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                          ...
                          {item.amount && (
                            <span className="flex items-center gap-1">
                              <FiDollarSign className="w-3 h-3" />${item.amount.toFixed(2)}
                            </span>
                          )}
                        </div>
                      </div>
