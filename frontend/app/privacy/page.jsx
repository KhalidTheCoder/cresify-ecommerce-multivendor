/**
 * @file page.jsx
 * @description Privacy policy page with server-rendered static content and structured sections.
...
import { Shield, Lock, Eye, Users, Globe, Bell, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function PrivacyPage() {
  ...
  const retentionData = Array.isArray(t('data_retention.table.rows', { returnObjects: true }))
    ? t('data_retention.table.rows', { returnObjects: true })
    : [];

  const tableHeaders = Array.isArray(t('data_retention.table.headers', { returnObjects: true }))
  ...
  const introductionContent = Array.isArray(t('introduction.content', { returnObjects: true }))
    ? t('introduction.content', { returnObjects: true })
    : [];

  const updatePoints = Array.isArray(t('updates.points', { returnObjects: true }))
      ...
      <section className="bg-gradient-to-r from-[#9838E1] to-[#F68E44] text-white py-16 px-6">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <p className="text-xl opacity-90 max-w-3xl mx-auto">
            {t('hero.description', 'Protecting your privacy is our top priority')}
          </p>
          <p className="mt-4 text-lg opacity-80">{t('hero.date', { date: formatDate() })}</p>
        </div>
      </section>
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="space-y-6">
          {privacyData.map((section, index) => {
            const IconComponent = section.icon ? iconComponents[section.icon] : Shield;
            ...
            return (
              ...
              >
                <button
                  onClick={() => toggleSection(section.id || `section-${index}`)}
                  ...
                  <div className="px-8 pb-8 pt-4 border-t border-gray-100">
                    <div className="grid md:grid-cols-3 gap-6">
                      {(Array.isArray(section.content) ? section.content : []).map(
                        (item, itemIndex) => (
                          <div key={itemIndex} className="bg-gray-50 p-5 rounded-lg">
                ...
                <tbody>
                  {retentionData.map((item, index) => (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="p-4 text-sm text-gray-600">{item.notes || 'Notes'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
