'use client';
import { useCreateServiceProviderMutation, useCreateUserMutation } from '@/feature/UserApi';
...
const Registerpage = () => {
  ...
  const [createUser, { isLoading: userLoading, error: userError }] = useCreateUserMutation();
  const [createServiceProvider, { isLoading: providerLoading, error: providerError }] =
  ...
  const yearsOfExperienceOptions =
    language === 'es'
      ? ['Menos de 1 año', '1-2 años', '2-5 años', '5-10 años', '10+ años', '15+ años', '20+ años']
      : [
          'Less than 1 year',
          '1-2 years',
          '2-5 years',
  ...
  const validateStep2 = () => {
    ...
    if (role === 'buyer') {
      if (!formData.country || !formData.region || !formData.city) {
        toast.error(
          language === 'es'
            ? 'Por favor, ingresa tu país, región y ciudad'
            : 'Please enter your country, region, and city',
        );
        return false;
      }
      return true;
    ...
    if (!formData.shopName) {
      toast.error(
        language === 'es'
          ...
          : 'Please enter shop/business name',
      );
      return false;
    }
    ...
    if (role === 'provider') {
      ...
      if (!formData.serviceCategory) {
        toast.error(
          language === 'es'
            ? 'Por favor, selecciona la categoría del servicio'
      ...
      if (!formData.serviceArea) {
        toast.error(
          language === 'es'
            ? 'Por favor, ingresa el área de servicio'
      ...
      if (!formData.serviceRedius) {
        toast.error(
          language === 'es'
            ? 'Por favor, selecciona el radio de servicio'
            : 'Please select service radius',
        );
        return false;
      }
      ...
      if (!formData.hourlyRate || parseFloat(formData.hourlyRate) <= 0) {
        toast.error(
          language === 'es'
            ? 'Por favor, ingresa una tarifa por hora válida'
      ...
      if (!formData.yearsOfExperience) {
        toast.error(
          language === 'es'
            ? 'Por favor, selecciona los años de experiencia'
      ...
      if (!formData.serviceDescription) {
        toast.error(
          language === 'es'
            ? 'Por favor, ingresa la descripción del servicio'
  ...
  const validateStep3 = () => {
    ...
    if ((role === 'seller' || role === 'provider') && !formData.nationalId) {
      toast.error(
        language === 'es' ? 'Por favor, ingresa DNI/NIF' : 'Please enter National ID/Tax ID',
      );
      return false;
    }
    ...
    if (role === 'provider' && formData.servicesImage.length === 0) {
      toast.error(
        language === 'es'
          ? 'Por favor, sube al menos una imagen del servicio'
          : 'Please upload at least one service image',
      );
      return false;
    }
    ...
    if (role === 'provider') {
      ...
      if (!formData.slotDuration || parseInt(formData.slotDuration) <= 0) {
        toast.error(
          language === 'es'
            ? 'Por favor, ingresa una duración de cita válida'
  ...
  const handleSubmit = async (e) => {
    ...
    if (role === 'seller' || role === 'provider') {
      ...
      if (formData.businessLogo) {
        try {
          toast.loading(
            language === 'es' ? 'Subiendo logo del negocio...' : 'Uploading business logo...',
          );
          ...
          toast.dismiss();
          toast.success(
            language === 'es' ? '¡Logo subido exitosamente!' : 'Logo uploaded successfully!',
          );
        } catch (error) {
          toast.error(
            language === 'es'
              ? 'Error al subir el logo. Por favor, intenta de nuevo.'
      ...
      if (role === 'provider') {
        ...
        if (formData.servicesImage.length > 0) {
          try {
            ...
            toast.success(
              language === 'es'
                ? '¡Imágenes del servicio subidas exitosamente!'
                : 'Service images uploaded successfully!',
            );
          } catch (error) {
            toast.error(
              language === 'es'
                ? 'Error al subir imágenes del servicio'
      ...
      if (response.error) {
        throw new Error(
          response.error.data?.message ||
            (language === 'es' ? 'Registro fallido' : 'Registration failed'),
        );
      }
  ...
  const renderProgressDots = () => (
    <div className="flex mb-8 items-center gap-[15px] justify-center w-full">
      <div className="flex items-center w-[35%]">
        ...
        <hr className={`w-[80%] ${currentStep >= 2 ? 'border-[#9838E1]' : 'border-gray-300'}`} />
      </div>
      <div className="flex items-center w-[35%]">
        <p
          className={`w-[35px] h-[35px] flex items-center justify-center rounded-full text-[1rem] ${
        ...
        <hr className={`w-[80%] ${currentStep >= 3 ? 'border-[#9838E1]' : 'border-gray-300'}`} />
      </div>
      <p
        className={`w-[35px] h-[35px] flex items-center justify-center rounded-full text-[1rem] ${
          currentStep >= 3
    ...
    <div className="bg-[#F6F1F4] min-h-screen py-8">
      ...
      <div className="flex md:px-16 max-w-5xl items-center min-h-[calc(100vh-4rem)] mx-auto gap-12">
        ...
        <section className="flex-1 justify-center hidden md:flex">
          <Image alt="Cresify Registration" src={image} className="max-w-full h-auto" />
        </section>
        ...
        <section className="flex-1">
          <div className="bg-white p-8 rounded-lg shadow-lg">
            ...
            <div className="flex justify-between items-center mb-4">
              <Image src={logo} width={150} height={60} alt="Cresify Logo" className="mb-2" />
              ...
              {currentStep === 1 && (
                <>
                  <div className="py-4 mb-4">
                    ...
                    <p className="text-[#9838E1]">{translate('step1Subtitle')}</p>
                  </div>
                  ...
                  <div className="py-4 mb-4">
                    <h4 className="text-xl font-semibold text-gray-800">
                      {translate('step2Title', {
                        roleDetails:
                          role === 'buyer'
                            ? translate('buyerLocation')
                            : role === 'provider'
                              ? translate('providerService')
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            required
                          />
                          <p className="text-xs text-gray-500 mt-1">{translate('ratePerHour')}</p>
                        </div>
                      ...
                      <div>
                        ...
                        >
                          ...
                          <option value="home-garden">
                            {language === 'es' ? 'Hogar y Jardín' : 'Home & Garden'}
                          </option>
                  ...
                  <div className="space-y-4">
                            ...
                            {formData.image ? (
                              <div className="flex flex-col items-center">
                                ...
                                <p className="text-sm text-green-600 font-medium">
                                  {language === 'es' ? 'Logo seleccionado:' : 'Logo selected:'}{' '}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          ...
                          <span className="text-xs text-gray-500 ml-1">
                            ({language === 'es' ? 'Mín. 1, máx. 10' : 'At least 1, max 10'})
                          </span>
                        </label>
                    ...
                    <div className="mt-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                      ...
                      <div className="text-sm text-gray-600 space-y-1">
                        <p>
                          <span className="font-medium">{translate('fullName')}</span>{' '}
                        ...
                        {role !== 'buyer' && (
                          <>
                            <p>
                              <span className="font-medium">{translate('business')}</span>{' '}
                              {formData.shopName}
                            </p>
                            {role === 'provider' && (
                              <>
                                ...
                                <p>
                                  <span className="font-medium">
                                    {translate('hourlyRateLabel')}
                                  </span>{' '}
                                  ${formData.hourlyRate}/hr
                                </p>
                                <p>
                                  <span className="font-medium">
                                    {translate('serviceAreaLabel')}
                                  </span>{' '}
                        ...
                        {role === 'provider' && (
                          <>
                            <p>
                              <span className="font-medium">{translate('workingDaysLabel')}</span>{' '}
                              {formData.workingDays.join(', ')}
                            </p>
                            <p>
                              <span className="font-medium">{translate('workingHoursLabel')}</span>{' '}
                              {formData.workingHoursStart} - {formData.workingHoursEnd}
                            </p>
                            <p>
                              <span className="font-medium">{translate('serviceImagesLabel')}</span>{' '}
                              {formData.servicesImage.length}{' '}
                              {language === 'es' ? 'subidas' : 'uploaded'}
                            </p>
                          </>
                        )}
                      </div>
            ...
            <div className="mt-4 text-center text-sm text-gray-500">
              {language === 'es' ? 'Paso' : 'Step'} {currentStep} {language === 'es' ? 'de' : 'of'}{' '}
              3
            </div>
          </div>
        </section>
