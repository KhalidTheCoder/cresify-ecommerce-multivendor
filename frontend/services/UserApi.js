/**
 * @file UserApi.js
 * @description RTK Query API slice for user authentication, registration and profile management.
...
export const UserApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getProviderBookingsForDate: builder.query({
      query: ({ providerId, date }) => `/provider-availability/${providerId}/bookings?date=${date}`,
    }),
    ...
    getProviderBookingStats: builder.query({
      query: ({ providerId, filter }) => `/booking/stats?providerId=${providerId}&filter=${filter}`,
    }),
  }),
});
