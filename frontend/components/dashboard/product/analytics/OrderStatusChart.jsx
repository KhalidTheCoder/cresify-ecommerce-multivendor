 ...
 * @description Pie chart breaking down orders by status for the vendor dashboard.
...
export default function OrderStatusChart() {
  return (
    <div className="bg-white rounded-[14px] p-6 border border-[#EEEAF5] shadow-[0_4px_22px_rgba(0,0,0,0.05)]">
      <h2 className="text-lg font-semibold text-[#1D1D1F] mb-4">Order Status Distribution</h2>
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        {data.map((item, index) => (
          <div key={index} className="flex items-center gap-2 text-sm">
            <span className="w-3 h-3 rounded-full" style={{ background: COLORS[index] }}></span>
            {item.name}
          </div>
        ))}
      </div>
