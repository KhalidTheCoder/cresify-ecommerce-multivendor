 ...
 * @description Admin overview table listing all platform bookings.
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-[#F0ECFF] p-4 md:p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[15px] font-semibold text-gray-900">Recent Bookes</h3>
        ...
        <table className="min-w-full text-left text-sm">
          ...
          <tbody>
            {bookings.map((row, idx) => (
              <tr
                key={row.id}
                className={`text-[13px] ${idx !== bookings.length - 1 ? 'border-b' : ''}`}
              >
                <td className="py-3 px-3 text-[#9C6BFF] font-medium">{row.id}</td>
