 ...
 * @description Product detail view with image carousel, variant selector, add-to-cart and wishlist toggle.
...
export default function ProductDetails({ id }) {
  ...
  // Wishlist queries
  const { data: wishlistStatus, isLoading: isCheckingWishlist } = useCheckWishlistQuery(
    { id, userId },
    { skip: !userId },
  );
    ...
    return (
      <section className="w-full bg-[#F7F7FA] py-10 px-6">
        <div className="max-w-[1300px] mx-auto">
          <h2 className="text-[20px] font-semibold text-[#1B1B1B] mb-6">{t('productDetails')}</h2>
      ...
      <div className="max-w-[1300px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="pr-4 pt-2">
            <h3 className="text-[26px] font-semibold text-[#1B1B1B] mb-1">{p.name}</h3>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                {p.seller?.email && (
                  <p className="text-[12px] text-gray-500 mt-1">{p.seller.email}</p>
                )}
              </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              <span className="text-[26px] font-semibold text-[#F78D25]">${p.price}</span>
              <span className="text-[16px] text-[#B0B0B0] line-through">${originalPrice}</span>
            ...
            {/* DESCRIPTION */}
            <h4 className="text-[15px] font-semibold text-[#1B1B1B] mb-2">{t('description')}</h4>

            <p className="text-[14px] text-[#6B6B6B] mb-7 max-w-[500px]">{p.description}</p>
            ...
            <div className="mb-8">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center border border-gray-300 rounded-lg">
                  ...
                  <span className="px-4 py-2 text-center min-w-[50px]">{qty}</span>
                  <button
                ...
                <span className="text-sm text-gray-500">
                  {p.stock > 0 ? `${p.stock} ${t('available')}` : t('outOfStock')}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 mb-10">
              ...
              >
                <Heart
                  size={19}
                  className={`${
                    isWishlisted ? 'text-[#A46CFF] fill-[#A46CFF]' : 'text-[#A46CFF]'
            ...
            {p.seller && (
              <div className="mt-8 pt-6 border-t border-gray-200">
                <h4 className="text-[15px] font-semibold text-[#1B1B1B] mb-3">{t('sellerInfo')}</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">{t('sellerName')}:</span>
                    <span className="ml-2 font-medium">{p.seller.name || p.seller.shopName}</span>
                  </div>
                  {p.seller.email && (
                    <div>
