/**
 * @file Navbar.jsx
 * @description Responsive top navigation bar with logo, links, cart badge, user dropdown and language switcher.
 * @component Navbar
 */
    ...
    <nav className="w-full bg-white border-b border-gray-100 sticky top-0 z-50">
              ...
              return (
                <Link
                  ...
                  className={`relative transition font-medium ${
                    isActive ? 'text-[#FF7A00]' : 'text-black hover:text-[#FF7A00]'
                  }`}
                >
                  {item.label}
                </Link>
      ...
      <div className="xl:hidden px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Hamburger */}
          {open ? (
            <FiX className="text-[26px] cursor-pointer" onClick={() => setOpen(false)} />
          ) : (
            <FiMenu className="text-[26px] cursor-pointer" onClick={() => setOpen(true)} />
          )}
        </div>
      </div>
