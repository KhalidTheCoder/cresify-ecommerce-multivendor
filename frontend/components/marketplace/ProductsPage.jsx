 ...
 * @description Product grid with sidebar filters and infinite scroll for the marketplace listing.
...
function ProductsPagePage({ searchTerm = '' }) {
  ...
  if (isFiltersApplied && selectedCategories.length > 0) {
    const categoryKeys = selectedCategories
      .map((cat) => {
        const categoryMap = {
          Fashion: 'fashion',
      ...
      <div className="max-w-[1350px] mx-auto flex gap-7">
        ...
        <aside className="hidden lg:block w-[270px]">
          <div className="bg-white rounded-[18px] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.06)] border border-[#F2EEF9]">
            ...
            {/* Categories */}
            <div className="mb-8">
              <p className="text-[14px] font-semibold text-[#1B1B1B] mb-3">{t('categories')}</p>
        ...
        <section className="flex-1">
          ...
          {(filters.country ||
            ...
            <div className="mb-5 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-100">
              <p className="text-sm font-semibold text-gray-700 mb-2">{t('active_filters')}</p>
          ...
          <p className="text-[13px] text-[#777] mb-5">
            {t('showing')} <span className="font-semibold text-[#444]">{products.length}</span>{' '}
            {t('of')} <span className="font-semibold text-[#444]">{totalProducts}</span>{' '}
            {t('products')}
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-white rounded-3xl">
              ...
              <h3 className="text-lg font-semibold text-gray-700 mb-2">{t('no_products_found')}</h3>
              <p className="text-gray-500 mb-4">
                {search ? `${t('no_products_for')} "${search}"` : t('no_products_available')}
                {(filters.country || filters.region || filters.city) && (
                  <span> {t('with_location_filters')}</span>
                )}
              </p>
          ...
          <div className="max-h-60 overflow-y-auto">
            {isLoading ? (
              ...
              <div className="px-4 py-8 text-center">
                {searchValue ? (
                  <>
                    <p className="text-sm text-gray-500">{getNoResultsMessage()}</p>
                    <button
                      className="mt-2 text-xs text-blue-600 hover:underline"
