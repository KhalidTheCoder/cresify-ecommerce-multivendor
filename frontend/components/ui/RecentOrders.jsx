 ...
 * @description Recent orders table with status badges and inline quick-action buttons.
    ...
    <div>
      ...
      <div className="mt-3 overflow-x-auto rounded-xl">
        <table className="min-w-full text-left text-[12px] md:text-[13px]">
          <thead>
            <tr className="text-[#9838E1] bg-[#F8F4FD] border-b border-[#F0EEF7] text-xs uppercase tracking-[0.12em]">
              <th className="py-6 pr-4 font-medium pl-2">{t('dashboard.tables.orderId')}</th>
              <th className="py-6 pr-4 font-medium pl-2">{t('dashboard.tables.amount')}</th>
              <th className="py-6 pr-4 font-medium pl-2">{t('dashboard.tables.sellerPayout')}</th>
              <th className="py-6 pr-4 font-medium pl-2">{t('dashboard.tables.status')}</th>
              <th className="py-6 pr-4 font-medium pl-2">{t('dashboard.tables.date')}</th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  <td className="pl-2 py-6 pr-4 font-medium">
                    {txn.orderId || `TXN-${txn._id?.slice(-6).toUpperCase() || 'N/A'}`}
                  </td>
                  ...
                  <td className="py-6 pr-4 text-[#8C8CA1]">{formatDate(txn.createdAt)}</td>
                </tr>
              );
            })}
          </tbody>
