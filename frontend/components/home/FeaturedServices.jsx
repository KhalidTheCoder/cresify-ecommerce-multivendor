/**
 * @file FeaturedServices.jsx
 * @description Grid of featured service listings with category icons.
...
export default function FeaturedServices() {
    ...
    if (data?.data) {
      ...
      const processedServices = limitedProviders.map((provider) => {
        ...
        return {
          ...
          reviews: 0,
          location:
            provider.city || provider.region || provider.country || 'Location not specified',
  ...
  if (error) {
    return (
      <section className="w-full py-20 px-10 bg-[#F5F5F7]">
        <div className="max-w-[1350px] mx-auto">
          <h2 className="text-[36px] font-bold text-center mb-2">{t('featured_services')}</h2>
      ...
      <div className="max-w-[1350px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {services.map((service) => (
            <Link key={service.id} href={`/service-details?id=${service.id}`} className="block">
              <div
                className="
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <h4 className="text-[14px] font-medium text-purple-600">{service.title}</h4>
                  </div>
                </div>
                ...
                <div className="flex items-center text-[14px] text-[#6A6A6A] mb-4">
                  {[...Array(5)].map((_, i) => (
                    <AiFillStar
                      key={i}
                      className={`text-[15px] ${
                        i < Math.floor(service.rating) ? 'text-[#FFA534]' : 'text-[#E0E0E0]'
                      }`}
                    />
                  ))}
                  <span className="ml-1">
