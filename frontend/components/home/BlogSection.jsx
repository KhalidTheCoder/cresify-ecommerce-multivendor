 ...
 * @description Latest blog article teasers with cover image and excerpt.
  ...
  return (
    <section className="w-full py-20 px-10">
      <div className="max-w-[1350px] mx-auto">
        {/* Heading */}
        <h2 className="text-[36px] font-bold text-center mb-2">{t('blog:latestArticles.title')}</h2>
          ...
          {posts.map((item, index) => (
            ...
            >
              ...
              <div className="flex justify-between items-center text-[13px] text-[#7A5FA6] font-medium mb-2 px-1">
                <span>{t(`blog:latestArticles.categories.${item.tag}`, item.tag)}</span>
              ...
              {/* Description */}
              <p className="text-[14px] text-[#6D6D6D] leading-[22px] mb-4 px-1">{item.desc}</p>
