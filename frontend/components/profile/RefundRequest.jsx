/**
 * @file RefundRequest.jsx
 * @description Refund request form and status tracker in the profile.
...
const RefundRequest = () => {
  ...
  const getProductNames = (items) => {
    ...
    return items.map((item) => item.product?.name || 'Product').join(', ');
  };
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6 px-2">
          <div className="text-sm text-gray-600">
            Showing {startIndex + 1} to {Math.min(endIndex, refunds.length)} of {refunds.length}{' '}
            requests
          </div>
