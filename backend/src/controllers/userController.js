 ...
 * @description HTTP request handlers for user authentication, registration, and profile management.
...
export const getSingleProvider = async (req, res) => {
  try {
    ...
    const slotDuration = availabilityDoc?.slotDuration || provider.slotDuration || 60;
    ...
    Object.entries(manualBlocked).forEach(([date, slots]) => {
      ...
      blockedSlotsMap[date] = [...new Set([...blockedSlotsMap[date], ...slots])];
    });
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (!workingDays.includes(dayName) || blockedDates.includes(dateStr)) continue;
