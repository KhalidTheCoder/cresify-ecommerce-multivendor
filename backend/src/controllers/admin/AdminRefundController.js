/**
 * @file AdminRefundController.js
 * @description Admin HTTP handlers for final-authority refund resolution.
 * @module controllers/admin/AdminRefundController
 */
...
export const reviewRefund = async (req, res) => {
  ...
  if (!refundId || !action || !adminId) return res.status(400).json({ message: 'Missing fields' });
  ...
  try {
    await session.withTransaction(async () => {
      ...
      if (
        ...
        try {
          ...
          if (payment.paymentId) {
            ...
            refundParams.payment_intent = String(payment.metadata.stripePaymentIntent);
      ...
      refund.status = refundAmount >= payment.amount ? 'refunded_full' : 'refunded_partial';
      refund.processedBy = new mongoose.Types.ObjectId(adminId);
      refund.processedAt = new Date();
      await refund.save({ session });
      ...
      if (sellers.length) {
        ...
        const wallets = await Wallet.find({ user: { $in: sellerIds } }).session(session);
        const walletByUser = new Map(wallets.map((w) => [w.user.toString(), w]));
        const walletBulkOps = [];
        const ledgerDocs = [];
        const txDocs = [];
        ...
        try {
          if (stripeRefund && stripeRefund.balance_transaction) {
            const bt = await stripe.balanceTransactions.retrieve(stripeRefund.balance_transaction);
        ...
        for (const b of sellers) {
          ...
          const deductFromReserved = toTwo(Math.min(beforeReserved, desiredDeduct));
          const remaining = toTwo(Math.max(0, desiredDeduct - deductFromReserved));
          ...
          if (remaining > 0) {
            ...
            txDocs.push({
              transactionId: `${payment._id}-refund-${sellerId}-curr-${Date.now()}`,
              type: 'refund',
              wallet: wallet._id,
              user: sellerId,
              order: order._id,
          ...
          if (feePolicy === 'seller' && totalStripeFee > 0) {
            const sellerShareFee = toTwo((totalStripeFee * (net || 0)) / totalNet);
            ...
            txDocs.push({
              transactionId: `${payment._id}-refund-fee-${sellerId}-${Date.now()}`,
              type: 'fee',
              wallet: wallet._id,
              user: sellerId,
              order: order._id,
          ...
          try {
            ...
            if (ov) {
              ...
              const commissionDeduct = toTwo((ov.commissionAmount || 0) * ratio);
              const commissionVATDeduct = toTwo((ov.commissionVATAmount || 0) * ratio);

              ov.amount = toTwo(Math.max(0, toTwo((ov.amount || 0) - amountDeduct)));
              ov.shippingAmount = toTwo(
                Math.max(0, toTwo((ov.shippingAmount || 0) - shippingDeduct)),
              );
              ov.commissionAmount = toTwo(
                Math.max(0, toTwo((ov.commissionAmount || 0) - commissionDeduct)),
              );
              ov.commissionVATAmount = toTwo(
                Math.max(0, toTwo((ov.commissionVATAmount || 0) - commissionVATDeduct)),
              );
              ov.commissionTotal = toTwo(
                Math.max(
                  0,
                  toTwo((ov.commissionTotal || 0) - toTwo(commissionDeduct + commissionVATDeduct)),
                ),
              );
              ov.sellerPayout = toTwo(
                Math.max(
                  ...
                  toTwo(
                    (ov.sellerPayout || 0) -
                      toTwo(
                        amountDeduct + shippingDeduct - (commissionDeduct + commissionVATDeduct),
                      ),
                  ),
                ),
              );
...
export const getRefund = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to fetch refund', error: err.message });
  }
};
  ...
  try {
    await session.withTransaction(async () => {
      const refund = await Refund.findById(refundId)
        ...
        .session(session);
      if (!refund) throw new Error('Refund not found');
      if (!refund.provider) return res.status(400).json({ message: 'Not a provider refund' });
      ...
      if (payment) {
        refund.status =
          refundAmount >= (payment.amount || 0) ? 'refunded_full' : 'refunded_partial';
        payment.refunds = payment.refunds || [];
        payment.refunds.push(refund._id);
        if (refund.status === 'refunded_full') payment.status = 'refunded';
        await payment.save({ session });
      ...
      try {
        if (refund.provider) {
          ...
          const wallet = await Wallet.findOne({ user: providerId }).session(session);
          if (wallet) {
            const beforeReserved = toTwo(wallet.reserved || 0);
            const beforeCurrent = toTwo(wallet.currentBalance || 0);
            const deductFromReserved = toTwo(Math.min(beforeReserved, refundAmount));
            const remaining = toTwo(Math.max(0, refundAmount - deductFromReserved));
            const newReserved = toTwo(beforeReserved - deductFromReserved);
            ...
            if (deductFromReserved > 0) {
              ...
              txDocs.push({
                transactionId: `${refund._id}-refund-prov-${providerId}-${Date.now()}`,
                type: 'refund',
                wallet: wallet._id,
                user: providerId,
                order: refund.order,
            ...
            if (remaining > 0) {
              ...
              txDocs.push({
                transactionId: `${refund._id}-refund-prov-curr-${providerId}-${Date.now()}`,
                type: 'refund',
                wallet: wallet._id,
                user: providerId,
                order: refund.order,
            ...
            if (ledgerDocs.length) await WalletLedger.insertMany(ledgerDocs, { session });
