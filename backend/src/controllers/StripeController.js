 ...
 * @description HTTP request handlers for Stripe payment intents and webhook event processing.
        ...
        if (stripeSession.metadata?.itemType === 'product') {
          ...
          if (!payment && stripeSession.metadata?.orderId) {
            ...
            const totalCommissionAmount = toTwo(
              parsedSellerBreakdown.reduce((sum, s) => sum + (s.commissionAmount || 0), 0),
            );
            const totalCommissionVATAmount = toTwo(
              parsedSellerBreakdown.reduce((sum, s) => sum + (s.commissionVATAmount || 0), 0),
            );
          ...
          const order = await Order.findById(new mongoose.Types.ObjectId(payment.order)).session(
            session,
          );
          if (!order) return;
          ...
          if (sellers.length) {
            ...
            for (const b of sellers) {
              ...
              const key = sellerId?.toString ? sellerId.toString() : String(sellerId);
            ...
            if (walletBulkOps.length) await Wallet.bulkWrite(walletBulkOps, { session });
          ...
          const providerPayout =
            payment.amount - (payment.commissionAmount || 0) - (payment.commissionVATAmount || 0);

          if (typeof providerPayout !== 'number' || isNaN(providerPayout)) {
            throw new Error(`Invalid provider payout for payment ${payment._id}`);
          }
