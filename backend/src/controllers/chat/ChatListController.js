/**
 * @file ChatListController.js
 * @description HTTP handlers for listing a user's active chat conversations.
 * @module controllers/chat/ChatListController
 */
...
export const myChatList = async (req, res) => {
  ...
  try {
    ...
    const bookings = await Booking.find({ customer: id }).select('provider');
    ...
    const uniqueProviders = [...new Map(providers.map((p) => [p._id.toString(), p])).values()];

    /* ======================
       2️⃣ SELLERS (Orders)
       ====================== */

    const orders = await OrderModel.find({ customer: id }).select('orderVendors');
    ...
    const uniqueSellers = [...new Map(sellers.map((s) => [s._id.toString(), s])).values()];
...
export const providerChatList = async (req, res) => {
  ...
  try {
    ...
    const uniqueCustomers = [...new Map(customers.map((c) => [c._id.toString(), c])).values()];
...
export const sellerChatList = async (req, res) => {
  try {
    ...
    const orderVendors = await OrderVendorModel.find({ seller: id }).select('order');
