/**
 * @file CreateRefundController.js
 * @description HTTP handlers for provider-initiated refund requests on behalf of service issues.
 * @module controllers/provider/CreateRefundController
 */
...
export const requestProviderRefund = async (req, res) => {
  try {
    const { providerId, bookingId, customerId, amount = 0, reason, evidence, paymentId } = req.body;
    if (!providerId || !bookingId || !customerId) {
      return res.status(400).json({ message: 'providerId, bookingId and customerId required' });
    }
    ...
    return res.status(201).json({ message: 'Provider refund requested', refund: refundDoc });
  } catch (err) {
