 ...
 * @description HTTP handlers for customer shopping cart — add, update, and remove items.
 * @module controllers/customer/CartController
 */
...
export const increaseCount = async (req, res) => {
  try {
    ...
    const updated = await Cart.findByIdAndUpdate(id, { $inc: { count: 1 } }, { new: true });
...
export const myCart = async (req, res) => {
  try {
        ...
        $addFields: {
          ...
          itemShipping: {
            $cond: [
              ...
              {
                $multiply: [{ $ifNull: ['$product.shippingCost', 0] }, '$count'],
              },
              0,
            ],
          },
    ...
    res.status(200).json({ message: 'Success', data: { groupedBySeller, totals } });
  } catch (error) {
    res.status(500).json({
      error,
      message: error?.message,
