 ...
 * @description HTTP handlers for customer order placement and tracking across multiple vendors.
 * @module controllers/customer/OrderController
 */
...
import { default as Order, default as OrderModel } from '../../models/OrderModel.js';
...
export const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const commissionVATRate = Number(process.env.PLATFORM_COMMISSION_VAT_PERCENT || 19);
    ...
    for (const p of products) {
      ...
      sellerMap[sellerId].gross = toTwo(sellerMap[sellerId].gross + productAmount);
      sellerMap[sellerId].shipping = toTwo(sellerMap[sellerId].shipping + productShipping);
    ...
    const sellerIds = Object.keys(sellerMap).map((id) => new mongoose.Types.ObjectId(id));
    ...
    for (const [sellerId, s] of Object.entries(sellerMap)) {
      ...
      const commissionVATAmount = toTwo((commissionAmount * commissionVATRate) / 100);
      ...
      totalCommissionVATAmount = toTwo(totalCommissionVATAmount + commissionVATAmount);
    ...
    res.status(201).json({
      message: `Order placed successfully (${(paymentMethod || 'cod').toUpperCase()})`,
      orderId: order._id,
    });
  } catch (err) {
  ...
  try {
    ...
    const agg = await Order.aggregate([
      ...
      // expose first paymentId for convenience
      {
        $addFields: {
          paymentId: {
            $arrayElemAt: [{ $map: { input: '$payments', as: 'p', in: '$$p._id' } }, 0],
          },
        },
      },
