/**
 * @file RefundController.js
 * @description HTTP handlers for customer refund request creation and status tracking.
 * @module controllers/customer/RefundController
 */
...
import { extractBase64FromDataURL, uploadImageToImgBB } from '../../utils/imageUpload.js';
...
export const requestRefund = async (req, res) => {
  try {
    const { paymentId, orderId, userId, sellerIds, reason, evidence, items } = req.body;
    ...
    const payment = await Payment.findById(new mongoose.Types.ObjectId(paymentId));
    ...
    if (Array.isArray(items) && items.length) {
      ...
      for (const [orderVendorId, its] of Object.entries(byVendor)) {
        ...
        for (const it of its) {
          ...
          let itemShipping = 0;
          if (prod?.shippingType === 'fixed') itemShipping = toTwo((prod.shippingCost || 0) * qty);
        ...
        let ev = its.flatMap((i) => (Array.isArray(i.evidence) ? i.evidence : []));
        // As a safety fallback, include global `evidence` if present and ev is empty
        if ((!Array.isArray(ev) || ev.length === 0) && Array.isArray(evidence)) {
    ...
    return res.status(201).json({ message: 'Refund requested', refunds: createdRefunds });
  } catch (err) {
    console.error('requestRefund error', err);
    return res.status(500).json({ message: 'Failed to request refund', error: err.message });
  }
};
...
export const listMyRefunds = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to list refunds', error: err.message });
  }
};
...
export const getRefund = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to get refund', error: err.message });
  }
};
