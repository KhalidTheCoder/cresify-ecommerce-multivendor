 ...
 * @description HTTP handlers for seller payout requests and payout history listing.
 * @module controllers/seller/PayoutController
 */
...
export const requestPayout = async (req, res) => {
  ...
  try {
    ...
    const currentBalance = toTwo(wallet?.currentBalance ?? wallet?.balance ?? 0);
    ...
    wallet.currentBalance = toTwo((wallet.currentBalance ?? wallet.balance ?? 0) - amount);
    ...
    session.endSession();
    return res.status(201).json({ message: 'Payout requested', payout: payout[0] });
  } catch (err) {
    console.error('requestPayout error', err);
    await session.abortTransaction();
    session.endSession();
    return res.status(500).json({ message: 'Failed to request payout', error: err.message });
  }
};
...
export const processPayout = async (req, res) => {
  ...
  try {
    ...
    const wallet = await Wallet.findOne({ user: payout.seller }).session(session);
    if (!wallet) {
      if (session.inTransaction && session.inTransaction()) await session.abortTransaction();
    ...
    if (payout.method === 'stripe_connect' && payout.metadata?.connectedAccountId) {
      try {
        ...
        wallet.currentBalance = toTwo((wallet.currentBalance ?? 0) + payout.amount);
        await wallet.save({ session });
        ...
        return res.status(502).json({
          message: 'Stripe transfer failed, payout marked as failed and funds returned',
          error: err.message,
          payout,
        });
      }
