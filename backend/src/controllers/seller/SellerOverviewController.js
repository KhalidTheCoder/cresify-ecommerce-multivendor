/**
 * @file SellerOverviewController.js
 * @description HTTP handlers for seller dashboard KPIs — revenue, order counts, and top products.
 * @module controllers/seller/SellerOverviewController
 */
  ...
  try {
    ...
    const monthlyBookingsData = await Booking.aggregate([
      {
        $match: {
          provider: providerId,
          $or: [{ status: 'completed' }, { status: 'accept' }],
        },
      },
      {
        $lookup: {
                ...
                { $ifNull: [{ $arrayElemAt: ['$providerDetails.hourlyRate', 0] }, 50] },
                {
                  $cond: {
                    ...
                    then: {
                      $divide: [
                        {
                          $toDouble: { $regexFind: { input: '$timeSlot.duration', regex: /^\d+/ } }
                            .match,
                        },
                        {
                          $cond: [
                            {
                              $regexMatch: {
                                input: { $toLower: '$timeSlot.duration' },
                                regex: /h$/,
                              },
                            },
                            1,
    ...
    const chartData = monthNames.map((monthName, index) => {
      const monthData = monthlyBookingsData.find(
        (data) => data._id.month === index + 1 && data._id.year === currentYear,
      );
    ...
    const recentBookings = completedBookings
      ...
      .map((booking) => ({
        ...
        amount:
          (booking.provider?.hourlyRate || 50) *
          (booking.timeSlot?.duration
            ? parseFloat(booking.timeSlot.duration.toString().replace(/[^0-9]/g, '')) / 60 || 1
            : 1),
      }));
    ...
    const paymentSummary = {
      ...
      pending: completedBookings.filter((b) => !b.paymentStatus || b.paymentStatus === 'pending')
        .length,
    };

    // 9️⃣ Service statistics
    const serviceStats = completedBookings.reduce(
      (stats, booking) => {
    ...
    const averageBookingValue =
      totalBookings > 0 ? Number((totalEarnings / totalBookings).toFixed(2)) : 0;
...
function calculatePendingPayout(bookings) {
  return bookings
    ...
    .reduce((sum, booking) => {
      ...
      const commission = bookingAmount * 0.1;
      const tax = commission * 0.19;
...
function calculateMonthlyGrowth(chartData) {
  ...
  const lastMonthEarnings =
    chartData[currentMonth - 1]?.value || chartData[chartData.length - 1]?.value || 0;
...
function getThisMonthBookings(bookings) {
  ...
  return bookings.filter((booking) => new Date(booking.createdAt) >= firstDayOfMonth).length;
}
...
function getThisWeekBookings(bookings) {
  ...
  return bookings.filter((booking) => new Date(booking.createdAt) >= firstDayOfWeek).length;
}
...
function getTodayBookings(bookings) {
  ...
  return bookings.filter((booking) => new Date(booking.createdAt) >= today).length;
}
