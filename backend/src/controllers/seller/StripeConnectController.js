/**
 * @file StripeConnectController.js
 * @description HTTP handlers for Stripe Connect seller onboarding and account link generation.
 * @module controllers/seller/StripeConnectController
 */
  ...
  try {
    ...
    if (!accountId) {
      try {
        ...
        const frontendReturn = returnUrl || process.env.FRONTEND_URL || 'http://localhost:3000';
  ...
  try {
    ...
    if (wallet.stripeAccountId) {
      try {
        ...
        const acct = await stripe.accounts.update(wallet.stripeAccountId, updatePayload);
        wallet.stripeDetails = acct;
        wallet.stripePayoutsEnabled = acct.payouts_enabled || false;
        wallet.stripeChargesEnabled = acct.charges_enabled || false;
        await wallet.save();
        ...
        console.warn('stripe.accounts.update failed', stripeErr?.message || stripeErr);
    ...
    return res.status(500).json({ message: 'Failed to update stripe account', error: err.message });
  }
};
...
export const setStripeFlags = async (req, res) => {
  try {
    ...
    if (typeof payoutsEnabled === 'boolean') wallet.stripePayoutsEnabled = payoutsEnabled;
    if (typeof chargesEnabled === 'boolean') wallet.stripeChargesEnabled = chargesEnabled;

    await wallet.save();
    return res.json({ wallet });
  } catch (err) {
    console.error('setStripeFlags error', err);
    return res.status(500).json({ message: 'Failed to set stripe flags', error: err.message });
  }
};
