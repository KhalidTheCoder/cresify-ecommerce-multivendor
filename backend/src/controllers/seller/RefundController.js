 ...
 * @description HTTP handlers for seller-side refund review — accept or reject customer requests.
 * @module controllers/seller/RefundController
 */
...
export const getSellerRefund = async (req, res) => {
  try {
    ...
    if (!refundId || !sellerId)
      return res.status(400).json({ message: 'refundId and sellerId required' });
...
export const respondToRefund = async (req, res) => {
  try {
    ...
    if (!id || !sellerId || !action) return res.status(400).json({ message: 'Missing fields' });
    ...
    return res.status(500).json({ message: 'Failed to respond to refund', error: err.message });
  }
};
