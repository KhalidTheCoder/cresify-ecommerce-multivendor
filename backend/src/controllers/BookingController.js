 ...
 * @description HTTP request handlers for the booking domain — creation, updates, and cancellation.
...
export const saveBooking = async (req, res) => {
  ...
  try {
    ...
    if (!timeSlotData) return res.status(404).json({ message: 'Time slot not found' });
...
export const getBookingStats = async (req, res) => {
  try {
    ...
    if (filter === 'today') {
      ...
      return res.status(400).json({ message: 'Invalid filter. Use: today | week' });
    }
